...
{
	return (
	   ((os_get_current_time_us() - dl->isdu_handler.isdu_timer) > IOLINK_ISDU_RESPONSE_TIME) &&
	   (dl->isdu_handler.isdu_timer != 0));
}
...
static void iolink_dl_mode_h_sm_startup2 (iolink_port_t * port)
{
   ...
   else if (dl->mode_handler.dl_mode == IOLINK_DLMODE_STARTUP)
   {
...
static void iolink_dl_message_h_sm_await_reply9 (iolink_port_t * port)
{
   ...
   else
   {
	   IOL_LOG_ERR (
         ...
         iolink_dl_mh_st_literals[dl->message_handler.state]);
      dl->cqerr  = 0;
      iolink_pl_get_error (port, &dl->cqerr);
   }
}
   ...
   bool write)
{
   ...
   dl->message_handler.rwcmd = (write) ? IOL_MHRW_WRITE : IOL_MHRW_READ;
    IOL_LOG_DBG("DL (%u): Message H triggered by DL_%s\n", iolink_get_portnumber (port), (write) ?
    "Write" : "Read");
...
static void iolink_dl_handle_error (iolink_port_t * port)
{
   ...
   if ((dl->cqerr & BIT (3)) != 0) // Frame handler timeout
   {
      dl->rxtimeout = true;
      IOL_LOG_WRN (
   ...
   if ((dl->cqerr & BIT (2)) != 0) // RChksmEr/FrameErr/ParityErr
   {
      dl->rxerror = true;
      IOL_LOG_WRN (
