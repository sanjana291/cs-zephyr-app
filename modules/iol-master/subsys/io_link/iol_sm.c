...
#include "iol_sm.h"
#include "iol_dl.h" /* DL_Read_req DL_Write_req DL_SetMode_req */
#include "iol_mw.h" /* AL_Read_req AL_Write_req */
#include "iol_pl.h" /* PL_SetMode_req */
#include "iol_main.h" /* iolink_fetch_avail_job, iolink_post_job, iolink_get_portnumber */
#include "osal.h"
#include "iol_log.h"
...
static const char * const iolink_fsm_sm_event_literals[] = {
   ...
   "CNF_COMLOST", /* T3, COMLOST when waiting on DL_{Read,Write}_cnf(),
                     MW_{Read,Write}_cnf() or DL_Write_Devicemode_cnf()*/
...
static void sm_MW_Read_cnf_cb (iolink_job_t * job);
   ...
   iolink_error_t errortype)
{
   ...
   job->mw_write_cnf.errortype = errortype;
   ...
   iolink_fsm_sm_event_t event)
{
   ...
   /* Not in spec */
   iolink_error_t res = MW_Write_req (
      port,
   ...
   {
      IOL_LOG_WRN (
         "SM (%u): MW_Write_req failed: %s\n",
         iolink_get_portnumber (port),
         iolink_error_literals[res]);
   }

   return SM_EVENT_NONE; /* Always wait for MW_Write_cnf() */
}
   ...
   iolink_fsm_sm_event_t event)
{
	...
	IOL_LOG_WRN (
      "SM (%u): ignore COMLOST wait for MW_{Read,Write}_cnf() or "
...
{
   iolink_job_t * job         = iolink_fetch_avail_job (port);
   job->mw_read_cnf.data      = data;
   job->mw_read_cnf.data_len  = len;
   job->mw_read_cnf.errortype = errortype;
...
static iolink_fsm_sm_event_t sm_checkSerNum (
   ...
   iolink_fsm_sm_event_t event)
{
   MW_Read_req (port, IOL_DEV_PARAMA_SERIAL_NUMBER, 0, sm_MW_Read_cnf);
...
static const iolink_fsm_sm_transition_t sm_trans_s0[] = {
   ...
   {SM_EVENT_SM_SetPortConfig_CFGCOM_or_AUTOCOM, SM_STATE_PortInactive, sm_portinactive},
   /* T15 */
...
static void sm_write_cycl_cnf_cb (iolink_job_t * job)
{
   iolink_sm_event (
      job->port,
      (job->mw_write_cnf.errortype != IOLINK_ERROR_NONE)
...
static void sm_MW_Read_cnf_cb (iolink_job_t * job)
{
   ...
   if(job->mw_read_cnf.errortype == IOLINK_ERROR_ADDRESS_INVALID)
   {
       memset (real_paramlist->serialnumber, 0, sizeof (real_paramlist->serialnumber));
       sm_check_sernum (port, true);
   }

   else if (job->mw_read_cnf.errortype != IOLINK_ERROR_NONE)
   {
      iolink_sm_event (
         port,
   ...
   {
      uint8_t len = job->mw_read_cnf.data_len;
      ...
      else
      {
         ...
         memcpy (real_paramlist->serialnumber, job->mw_read_cnf.data, len);
         sm_check_sernum (port, true);
      }
   }
}
...
#define MIN_CYCL_TIME_32MS 0x80
      ...
      IOL_LOG_INF ("SM (%u): param MIN_CYCL  = 0x%02x\n",
                   iolink_get_portnumber (port), value,
                   real_paramlist->cycletime);
      break;
   case IOL_DIR_PARAMA_MSEQ_CAP:
      sm->dev_com.mseq_cap = value;
      IOL_LOG_INF ("SM (%u): param MSEQ_CAP = 0x%02x\n",
                   iolink_get_portnumber (port), value);
      break;
   case IOL_DIR_PARAMA_REV_ID:
      real_paramlist->revisionid = value;
      IOL_LOG_INF ("SM (%u): param REV_ID   = 0x%02x\n",
                   iolink_get_portnumber (port), value);
      break;
   case IOL_DIR_PARAMA_PDI:
      sm->dev_com.pdi = value;
      IOL_LOG_INF ("SM (%u): param PDI      = 0x%02x \n",
                   iolink_get_portnumber (port), value);
      break;
   case IOL_DIR_PARAMA_PDO:
      sm->dev_com.pdo = value;
      IOL_LOG_INF ("SM (%u): param PDO      = 0x%02x \n",
                   iolink_get_portnumber (port), value);
      break;
   case IOL_DIR_PARAMA_VID_1:
      real_paramlist->vendorid = value << 8;
      IOL_LOG_INF ("SM (%u): param VID_1    = 0x%02x\n",
                   iolink_get_portnumber (port), value);
      break;
   case IOL_DIR_PARAMA_VID_2:
      real_paramlist->vendorid |= value;
      IOL_LOG_INF ("SM (%u): param VID_2    = 0x%02x => vendorid=0x%04x\n",
                   iolink_get_portnumber (port), value,
                   real_paramlist->vendorid);
      break;
   case IOL_DIR_PARAMA_DID_1:
      real_paramlist->deviceid = value << 16;
      IOL_LOG_INF ("SM (%u): param DID_1    = 0x%02x\n",
                   iolink_get_portnumber (port), value);
      break;
   case IOL_DIR_PARAMA_DID_2:
      real_paramlist->deviceid |= value << 8;
      IOL_LOG_INF ("SM (%u): param DID_2    = 0x%02x\n",
                   iolink_get_portnumber (port), value);
      break;
   case IOL_DIR_PARAMA_DID_3:
   ...
   case IOL_DIR_PARAMA_FID_1:
      ...
      IOL_LOG_INF ("SM (%u): param FID_1    = 0x%02x\n",
                   iolink_get_portnumber (port), value);
      break;
   case IOL_DIR_PARAMA_FID_2:
      real_paramlist->functionid |= value;
      IOL_LOG_INF ("SM (%u): param FID_2    = 0x%02x => functionid=0x%04x\n",
                   iolink_get_portnumber (port), value,
                   real_paramlist->functionid);
      break;
   default:
      CC_ASSERT (0);
      break;
   ...
   default:
	   IOL_LOG_ERR(
	      "SM (%u): DL_Read_cnf in UNKNOWN state %d"
	      " addr=0x%02x val=0x%02x stat=%d\n",
	      iolink_get_portnumber(port),
	      sm->state,
	      job->dl_rw_cnf.addr,
	      job->dl_rw_cnf.val,
...
static void sm_DL_Write_cnf_cb (iolink_job_t * job)
{
   ...
   else
   {
      ...
      default:
   	   IOL_LOG_ERR(
   	      "SM (%u): DL_Write_cnf in UNKNOWN state %d"
   	      " addr=0x%02x val=0x%02x stat=%d\n",
   	      iolink_get_portnumber(port),
   	      sm->state,
   	      job->dl_rw_cnf.addr,
